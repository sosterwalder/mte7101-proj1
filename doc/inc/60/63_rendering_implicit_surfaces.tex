...
$L_{\text{diffuse}}$ die Farbe des eingehenden diffusen Lichtes ist. Die
Richtung der Lichtquelle, ausgehend von Punkt $\bm{x}$, ergibt sich durch das
Punktprodukt zwischen der Einheitsnormalen $\bm{n}$ des Punktes und dem
Einheitsvektor $\bm{l}$.
...
\begin{gather}
    I_{\text{specular}} = c_{\text{facing}} \cdot k_{s}(\bm{x}) \cdot L_{\text{specular}} \cdot \max{(\bm{n} \cdot \bm{h}, 0)}^{k_{e}}
\end{gather}
...
ist. Der Faktor $c_{\text{facing}}$ definiert, ob die Oberfläche
...
Sichtbarkeitsfunktion $\Gamma$ in Abhängikeit der euklidischen Distanz
...
\begin{gather}
    ...
    d = |r_{n} - r_{0}| \\
    \varepsilon = \Gamma_{alpha, \gamma}(|r_n - r_0|)
\end{gather}
