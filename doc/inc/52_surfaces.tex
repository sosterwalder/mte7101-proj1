...
schneidet dieses auch nicht ($\overline{\overset{\circ}{\bm{A}}}$) --- nur der
äusserste Punkt des Abstandes (also des Radius der Kugel) liegt genau auf der
Oberfläche des Objektes ($\partial \bm{A}$). Der Radius solch einer Kugel wird
durch Evaluation der Distanzfunktion eines abzutastenden Punktes im Raum
bestimmt.\\
\\
...
\\
Evaulaiert man nun die Gleichung $F(t)$ unter Anwendung der eben beschriebenen
...
(``unbounding spheres''), welche durch die rekrusive Gleichung
...
\end{lstlisting}
\footnotetext{Algorithmus in Pseudocode
    gemäss~\cite{hart_sphere_1994}[S. 531, Fig. 1]}
